...
- Only wake a full OpenClaw worker run when the watchdog emits a `NEW` active task or `STALL`.
...
- For `NEW` task rows from `heyarp tasks --next --json` and for `STALL`, start or resume a real worker run through `arp-worker-run-openclaw.js`; the watchdog itself must not merely queue the event and stop.
