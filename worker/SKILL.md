...
```text
Normal: Windows Task Scheduler -> Node SSE daemon -> inbox event? -> one watchdog tick -> start OpenClaw worker run per executable order
                                                      reconcile every 30s -> watchdog tick
                                                               |
                                                               +-> offered order? accept inline, then wait for buyer funding
                                                               +-> funded executable order? start OpenClaw worker run
