...
- **One worker run per executable order.** The watchdog handles cheap protocol steps itself (handshake accept and default delegation accept). It starts Claude Code only when the job is funded/actionable, so buyer funding delays do not consume runner capacity.
