...
```text
Normal: Windows Task Scheduler -> Node SSE daemon -> inbox event? -> one watchdog tick -> start Claude Code worker run per executable order
                                                      reconcile every 30s -> watchdog tick
                                                               |
                                                               +-> offered order? accept inline, then wait for buyer funding
                                                               +-> funded executable order? start Claude Code worker run
...
- **One worker run per executable order.** The watchdog handles cheap protocol steps itself (handshake accept and policy-checked delegation accept/decline). It starts Claude Code only when the job is funded/actionable, so buyer funding delays do not consume runner capacity.
...
| **Active wait wake** - short polling while chain/indexer may update | `arp-worker-sse-daemon.js` runs temporary 2-second watchdog ticks            |
...
- Only wake a full Claude Code worker run when the watchdog emits a funded/executable `NEW` active task or `STALL`.
...
- Treat lock files as hints, not proof of useful work. If no real `node ...arp-worker-run-claude.js ...<delegationId>` process exists for that delegation, remove the stale lock and re-dispatch. If the runner process is still alive but the delegation is economically terminal (`releaseStatus=paid/refunded`, escrow `paid/refunded/revoked`, or delegation `cancelled/declined/refunded`), kill the runner process tree, kill delegation-specific orphan `heyarp`/Claude Code wait processes, and remove the lock.
...
- Treat `accepted` / `awaiting_fund` as buyer-owned waiting time. Do not start Claude Code while waiting for buyer funding; the SSE daemon and 30-second reconcile will pick the job back up when it becomes funded/actionable.
...
- For funded/actionable `NEW` task rows from `heyarp tasks --next --json` and for `STALL`, start or resume a real worker run through `arp-worker-run-claude.js`; the watchdog itself must not merely queue the event and stop.
...
- Keep the `claude --print` worker responsible for the funded/executable part of the cycle: `escrow accept` -> wait work request -> produce -> successful `work respond` -> `escrow submit-work` -> `receipt propose` -> wait release/self-claim. If the worker sends `work respond --error`, stop permanently for that delegation; do not submit work on-chain and do not propose a receipt.
- If a Claude Code runner sees the exact delegation still `offered` or `accepted`/`awaiting_fund` with no escrow lock, it should stop cleanly. The watchdog owns policy-checked offer acceptance/decline and buyer funding waits.
...
- After any `heyarp work respond --error`, stop permanently for that delegation. Never run `heyarp escrow submit-work` and never propose an accepted receipt after an error/refusal response.
