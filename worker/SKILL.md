...
- Only wake a full Hermes worker run when the watchdog emits `NEW delegation`, `NEW work_request`, or `STALL`.
...
Invoke-WebRequest -UseBasicParsing 'https://raw.githubusercontent.com/RealWagmi/heyarp-install-windows/main/worker/arp-worker-run-hermes.js' -OutFile (Join-Path $workerSkill 'arp-worker-run-hermes.js')
```
...
- For `NEW delegation`, `NEW work_request`, and `STALL`, start or resume a real worker run through `arp-worker-run-hermes.js`; the watchdog itself must not merely queue the event and stop.
