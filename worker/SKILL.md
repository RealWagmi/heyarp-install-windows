...
description: Run an ARP worker through Hermes on Windows with Task Scheduler, an SSE daemon, a funded-only watchdog, and one resumable Hermes run per funded delegation. Supports primary delegation deliverables, revisions, Solana, and EVM.
---
...
heyarp selftest --role worker --skills-dir "$env:LOCALAPPDATA\hermes\skills"
```
...
- **One worker run per funded order.** The watchdog handles handshakes and static offer policy itself. Before starting Hermes it positively verifies the delegation is funded and the escrow is in an actionable state. Unknown or unfunded states fail closed.
...
- Only wake Hermes after positive proof of buyer funding. A server task row by itself is not enough.
...
- Treat lock files as hints, not proof of progress. Stale locks are removed. Terminal cleanup includes `failed`, `revoked`, `dispute_resolved`, and `dispute_closed`. Operators may configure a positive maximum runtime as an emergency cap for a live but hung Hermes process; the default has no fixed lifetime.
...
Invoke-WebRequest -UseBasicParsing 'https://raw.githubusercontent.com/RealWagmi/heyarp-install-windows/hermes/worker/arp-worker-preflight.js' -OutFile (Join-Path $workerSkill 'arp-worker-preflight.js')
...
$maxRuntimeMinutes = 0 # Keep the same Hermes process for the full delegation lifecycle.
...
$hermesRunner = Join-Path $workerSkill 'arp-worker-run-hermes.js'
$hermesPath = (& node -e "const runner = require(process.argv[1]); process.stdout.write(runner.resolveHermes());" $hermesRunner).Trim()
if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($hermesPath)) {
  throw 'Could not resolve a usable Hermes executable for the scheduled worker.'
}
$monitorArgs = "`"$hiddenLauncher`" --workspace `"$workspace`" --state-root `"$stateRoot`" --from-did `"$fromDid`" --heyarp-home `"$heyarpHome`" --hermes-path `"$hermesPath`"$policyArgs --max-jobs `"$maxJobs`" --max-runtime-minutes `"$maxRuntimeMinutes`" --reconcile-seconds 30 --active-poll-seconds 2 --active-window-seconds 120"

$preflight = Join-Path $workerSkill 'arp-worker-preflight.js'
$preflightArgs = @($preflight, '--heyarp-home', $heyarpHome, '--from-did', $fromDid, '--hermes-path', $hermesPath)
foreach ($policy in $acceptPolicies) {
...
- Treat `accepted` / `awaiting_fund` as buyer-owned waiting time. Do not start Hermes while waiting for buyer funding. Before every `NEW`/`STALL` launch, read the exact delegation plus escrow and dispatch only when the delegation is funded and escrow is `created`, `in_progress`, `submitted`, or `disputing`.
...
- Process `offered` / `awaiting_acceptance` rows inline only after exact amount and exact network-qualified asset match. This is a static policy check; the funded Hermes run performs semantic preflight before staking.
- For funded/actionable `NEW` task rows from `heyarp tasks --next --json`, dispute-monitor recovery rows, and `STALL`, start or resume a real worker run through `arp-worker-run-hermes.js`; the watchdog itself must not merely queue the event and stop.
...
$pinnedHermesArgument = "--hermes-path `"$hermesPath`""
if (-not $taskArguments.Contains($pinnedHermesArgument)) {
  throw "Scheduled task does not pin the expected Hermes executable: $hermesPath"
}
...
try {
  ...
  $selftest = heyarp selftest --role worker --skills-dir "$env:LOCALAPPDATA\hermes\skills" --json | ConvertFrom-Json
...
Mirror of the buyer flow, "my-turn" side. One Hermes process owns the funded delegation until economic terminal state or a definitive worker error/refusal. After every action it re-reads live state and continues from the next pending step. While the buyer or chain owes the next move, use `heyarp status <rel-id> --wait --wait-timeout 300 --json` **without `--until`**. The default wait wakes when this worker owns the next action or the cycle terminates; a timeout exit code `124` means re-read state and continue the same loop.
...
- The Windows design intentionally starts Hermes only after buyer funding. If primary preflight then fails, do not stake, do not use `work respond --error`, write `<state-root>\logs\<delegation-id>.refusal.txt`, and let the buyer cancel the untouched lock. The watchdog treats that file as a durable no-redispatch marker so it does not repeatedly start agents for the same refused primary.
