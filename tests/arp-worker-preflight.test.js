...
test('preflight validates accepted Solana and EVM rails', async () => {
  ...
  try {
    const result = await preflight({
      ...
      resolveClaude: (options) => {
        assert.equal(options.explicitPath, claudePath);
        return claudePath;
      },
...
test('preflight rejects a missing or unusable Claude Code CLI', async () => {
  await assert.rejects(
    preflight({
      'heyarp-home': 'C:\\worker-home',
      'from-did': workerDid,
