...
test('preflight validates accepted Solana and EVM rails', async () => {
  ...
  try {
    const result = await preflight({
      ...
      resolveCodex: (options) => {
        assert.equal(options.explicitPath, codexPath);
        return codexPath;
      },
    ...
    assert.equal(result.codexPath, codexPath);
...
test('preflight rejects a missing or unusable Codex CLI', async () => {
  await assert.rejects(
    preflight({
      'heyarp-home': 'C:\\worker-home',
      'from-did': workerDid,
