...
test('SSE daemon forwards the pinned OpenClaw path to startup preflight', () => {
  ...
  const pathIndex = invocation.args.indexOf('--openclaw-path');
  assert.notEqual(pathIndex, -1);
  assert.equal(invocation.args[pathIndex + 1], openclawPath);
});
