...
test('SSE daemon forwards the pinned Claude path to startup preflight', () => {
  const claudePath = 'C:\\Tools\\claude.cmd';
  let invocation;
  ...
  const pathIndex = invocation.args.indexOf('--claude-path');
  assert.notEqual(pathIndex, -1);
  assert.equal(invocation.args[pathIndex + 1], claudePath);
});
